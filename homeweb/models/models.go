...
func init()  {
	...
	//设置数据库
	orm.RegisterDataBase("default","mysql","root:131206@tcp("+utils.G_mysql_addr+":"+utils.G_mysql_port+")/micro?charset=utf8")
	//orm.RegisterDataBase("default","mysql","root:131206@tcp(192.168.0.112:3306)/micro?charset=utf8")
	//注册model
...
type User struct {
	...
	Mobile string `orm:"size(11);unique" json:"mobile"` //手机好
	...
	Orders []*OrderHouse `orm:"reverse(many)" json:"orders"` //用户下的订单  一人多次订单
}
...
type House struct {
	...
	Area *Area `orm:"rel(fk)" json:"area_id"` //归属地区域编号 与地区表进行关联
	Title string `orm:"size(64)" json:"title"` //房屋标题
	Price int `orm:"default(0)" json:"price"`  //单价  单位分 每次价格乘以100
	...
	Max_days int `orm:"default(0)" json:"max_days"`  //最多入住天数 0表示不限制
	Order_count int `orm:"default(0)" json:"order_count"`  //预定完成的该房屋的订单数
	Index_image_url  string `orm:"size(256)" orm:"default("")" json:"index_image_url"` //房屋主图路径
	Facilities []*Facility `orm:"reverse(many)" json:"facilities"` //房屋设施 与设施表进行关联
	Images []*HouseImage `orm:"reverse(many)" json:"img_urls"`  //房屋的图片 除主图之外的所有图片
	Orders []*OrderHouse  `orm:"reverse(many)" json:"orders"`  //房屋订单  与房屋表进行管理
	Ctime time.Time  `orm:"auto_now_add;type(datatime)" json:"ctime"` //创建时间
}
...
func (this *House)To_one_house_desc() interface{}  {
	...
	for _, facility := range this.Facilities {
		facilites = append(facilites,facility.Id)
	}

	house_desc["facilites"] = facilites
	//评论用户
	comments := []interface{}{}
	orders := []OrderHouse{}
	o := orm.NewOrm()
...
//区域信息 table_name = area 需要我们手动添加到数据库中
type Area struct{
	Id int `json:"aid"`   //区域编号  1  2
	Name string `orm:"size(32)" json:"aname"`  //区域名称  昌平 海淀
	Houses []*House  `orm:"reverse(many)" json:"houses"`  //区域所有的房屋	与房屋表进行关联
}
//设施信息 table_name=facility  //设施信息需要我们提前手动添加
type Facility struct {
	Id int `json:"fid"`  //设施编号
	Name string `orm:"size(32)"` //设施名称
	House []*House `orm:"rel(m2m)"`  //都有哪些房屋有此设施 与房屋表进行关联
}
//房屋图片 table_name = home_image
...
func (this *OrderHouse) To_order_info() interface{} {
	order_info := map[string]interface{}{
		...
		"end_date"  : this.Begin_date.Format("2006-01-02 15:04:05"),
		"ctime"  : this.Ctime.Format("2006-01-02 15:04:05"),
		"days"  : this.Days,
		"amount"  : this.Amount,
		"status"  : this.Status,
